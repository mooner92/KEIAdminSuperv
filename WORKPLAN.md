...
| **P4 배포** | 두 화면 사내 공개 | `kei-rag-api`, 웹앱(Next.js+TDS, 비서/둘러보기/그래프), Zero Trust | 사내에서 두 화면 접속 가능 |
...
### 목표
두 화면을 사내에 띄운다. 한 웹앱(`web/`, Next.js 14 + TDS, `output:export`)이 **비서(/) · 둘러보기(/browse) · 관계 그래프(/graph)**를 모두 제공하고, 백엔드 `kei-rag-api`(127.0.0.1:9000)가 검색·생성·인증·채팅을 담당한다. 둘 다 같은 오리진(PM2 server.js 리버스 프록시)으로 묶여 Cloudflare Zero Trust 뒤에 둔다. (Open WebUI는 `/v1/chat/completions`로 등록 가능한 옵션으로 유지.)
...
| 화면 | 경로 | 내용 |
|---|---|---|
| 비서 | `/` | 로그인 후 멀티턴 RAG 채팅 + 우측 메시지별 근거 패널 + Notion형 문서 드로어 |
...
### P4-a · [비서] RAG API 실행 (kei-rag-api)
...
- 비서/둘러보기/관계 그래프 3화면이 한 앱. `server.js`가 정적 `out/`와 `/api/rag/*`·`/api/app/*`(→127.0.0.1:9000) 리버스 프록시를 같은 오리진으로 묶는다(RAG API LAN 비노출).

### P4-d · Cloudflare Zero Trust 라우트

- 웹앱(PM2 server.js, 3100)을 **기존 Cloudflare Tunnel**에 라우트로 추가(비서/둘러보기/그래프 + `/api/*` 한 오리진)
...
- 웹앱 `out/` 정적 사이트가 PM2 server.js(3100)로 서빙됨(비서/둘러보기/그래프 + RAG API 리버스 프록시)
- Cloudflare Tunnel 라우트(웹앱 한 오리진) + Access 정책

### 완료의 정의 (DoD)
- [x] 비서 화면에서 로그인 → 멀티턴 RAG 질문 → 출처/면책 포함 답변(스트리밍) 수신, 메시지별 근거·채팅기록 영속
