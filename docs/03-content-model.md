...
| 진실원천 | `20_규정원문/` | HWP/HWPX 변환 결과 | KEI 규정 원문(번호 1000~6000). **의역 금지**, 표/오타만 검수 | `regulation` | 제N조 = 1청크 |
...
## 3. KEI 규정번호 분류 (1000~6000)

`20_규정원문/` 의 하위 폴더는 **규정번호 첫 자리**로 자동 결정된다. 아래 표는 `01_hwp_to_md.py` 의 `CATEGORY` 딕셔너리와 **1:1로 일치**한다.
...
| `4` | `4000_보수·여비` | 보수·여비 |
| `5` | `5000_연구·정보` | 연구·정보 |
| `6` | `6000_총무·보안·회계` | 총무·보안·회계 |
| `7` | `6000_총무·보안·회계` | **7xxx(회계/구매)도 총무·보안·회계로 합류** |

> [!note] 7xxx → 6000 흡수
> 7번대(회계/구매 계열) 규정은 별도 폴더를 만들지 않고 `6000_총무·보안·회계` 로 배치한다. 코드 상으로도 `"7": "6000_총무·보안·회계"` 로 매핑되어 있다.

번호를 추정할 수 없는 파일(첫 자리 미매칭)은 `CATEGORY.get(..., "")` 가 빈 문자열을 반환하므로 `20_규정원문/` 루트에 떨어진다. 이런 경우는 검수 단계에서 폴더를 수동 교정한다.

> [!todo] 확인 필요: 실제 규정 번호·제목 목록
> 위 표는 **첫 자리 → 폴더** 매핑 규칙만 정의한다. 각 폴더에 들어갈 구체 규정 번호/제목/내용은 원문 HWP를 변환해 확정한다(이 문서에서 임의로 단정하지 않는다).
...
`01_hwp_to_md.py` 의 `parse_filename()` 이 원본 HWP 파일명에서 다음을 추출한다.

| 요소 | 추출 규칙 | 예시(형식 설명용) |
|------|-----------|-------------------|
| 번호 | 앞쪽 3~4자리 숫자 `re.match(r"(\d{3,4})")` | 4자리 번호 |
| 제목 | 번호·확장자·괄호 이후를 제거한 나머지 | 규정명 |
| 개정일 | `_YYMMDD` → `20YY-MM-DD` 로 정규화 | `_250324` → `2025-03-24` |
...
---
type: regulation
규정번호: "4300"          # 형식 예시 — 실제 번호는 원문에서 확정
규정명: "○○규정"
분류: "4000_보수·여비"
개정일: 2025-03-24
...
각 청크는 아래 5개 필드를 메타로 싣는다. `col.upsert(..., metadatas=...)` 가 사용하는 키와 동일하다.

| 키 | regulation | guide / term |
|----|-----------|--------------|
| `규정명` | 메타 `규정명`(없으면 파일 stem) | `제목` 또는 `용어` |
| `규정번호` | 메타 `규정번호` | `""`(빈 값) |
| `조` | `article_no()` 결과 = `제N조` | `""`(빈 값) |
| `type` | `regulation` | `guide` / `term` |
| `path` | 노트 절대경로 `str(md)` | 동일 |

`article_no()` 는 청크 머리에서 `제\s*(\d+)\s*조` 를 잡아 `제N조` 문자열을 만든다. 이 값이 RAG 답변 출처 `[규정명 제N조]` 의 조문 부분이 된다(검색·답변 흐름은 [05-rag-design.md](05-rag-design.md)).

> [!warning] 02 코드와의 동기화는 계약이다
> §3 분류표는 `01` 의 `CATEGORY`, §7 메타 키는 `02` 의 `upsert` 와 **글자 단위로 맞춰야** 한다. 코드가 바뀌면 이 문서를, 이 문서 규약이 바뀌면 코드를 같이 고친다. 변환·청킹 파이프라인 전체는 [04-pipeline.md](04-pipeline.md) 참고.
