...
> [!note]
> **실측 진행(2026-06-19, 개발 머신).** M1~M3 핵심 경로가 실제 데이터로 동작합니다: HWP/HWPX **111개 변환**(1건 timeout 격리), 조문 단위 **3044 청크** 임베딩(`kei_regs`), 검색 회수가 정확합니다(예: "출장 여비 정산" → 여비규정 제9조). 답변 **생성**만 A40 vLLM 서버 연결을 기다립니다. 관련 커밋: `6750748`(변환)·`9226c42`(임베딩)·`fa18378`(검색/API).
...
3. **vLLM 연결·답변 생성 검증** — A40 서버 vLLM 엔드포인트 연결 후 생성 단계 검증(M3 마무리).
...
- **[비서]는 M3(통제형 API)에 의존.** 채팅은 그림이 아니라 텍스트+임베딩 검색으로 답하므로, 색인(M2)과 출처 강제 API(M3)가 선행돼야 합니다. 현재 검색·근거주입·출처는 검증됐고, 마지막 **답변 생성**만 A40 vLLM 서버 연결을 기다립니다.
...
> [!note]
> **검색·근거주입·출처는 검증됨, 답변 생성은 vLLM 대기.** 이 개발 환경(2× Quadro RTX 6000)엔 vLLM이 미기동이라 실제 "답변 생성" 단계는 미검증입니다. 생성 검증은 타깃 배포 GPU(A40 48GB)의 vLLM 엔드포인트 연결 후 진행합니다. 검색/API는 실행 완료(커밋 `fa18378`).
