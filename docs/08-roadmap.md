...
| **M2 — 색인(임베딩)** | [P2](../WORKPLAN.md#p2--청킹임베딩) | 제N조/헤딩 단위 청킹 + 벡터 색인 | Chroma 컬렉션 `kei_regs` | `02`로 색인, `03`으로 질의 응답 확인 | ✅ 완료(약 4,418 청크, 별표 1급 청크·하위청킹 반영) |
| **M3 — 통제형 RAG API** | [P3](../WORKPLAN.md#p3--rag-질의평가) | 출처 강제·가드레일·인증·멀티턴·스트리밍 API | `tools/04_rag_api.py`(OpenAI 호환 `/v1/*` + 인증/채팅 `/app/*`) | `/v1/chat/completions`가 출처 포함 응답, `/app/*`가 멀티턴 SSE 응답 | ✅ 완료(검색·리랭커·생성·인증·멀티턴·스트리밍) |
...
| **M5 — 검수·운영 안착** | [P5](../WORKPLAN.md#p5--운영유지보수-순환) | 검수 워크플로·접근통제·운영·자기개선 루프 | 검수 완료 콘텐츠, Zero Trust 라우트, 운영 문서, 운영자 대시보드 | 행정 초보가 출처 달린 답을 신뢰하고 사용 | 🔶 진행(검수 도구·피드백·대시보드 완료, 전건 검수·접근통제 대기) |
| **품질 강화(P1.1~P1.5·P2.1~P2.5)** | [품질강화](12-품질강화.md) | 운영 중 검색·생성·신뢰·운영 품질 끌어올리기 | 평가 하베스트, 리랭커, 멀티턴 재작성, 검수 큐, 피드백 루프, 금액 신뢰, 하위청킹, ERP·서식 연결, 운영자 대시보드 | 지표(strict Hit@1·면책 보장 등) before/after 개선 | ✅ 완료(P1.3 VLM 표복원만 보류) |

> [!note]
> **실측 진행(2026-06-21, 개발 머신).** M1~M3 전 경로가 실제 데이터로 동작합니다: 4개 섹션 **271문서**(규정집 111 · 연구행정 가이드 64 · 용어집 84 · ERP 시스템 12) + 교차링크([[ ]] 그래프 엣지), 헤딩/제N조 단위 **약 4,418 청크** 임베딩(`kei_regs`, 별표 1급 청크·긴 청크 하위청킹 반영), 검색 회수가 정확합니다(예: "출장 여비 정산" → 여비규정 제9조). 검색은 밀집(KURE-v1) top-20 → 리랭커(bge-reranker-v2-m3) 재점수 → top-5(strict Hit@1 0.600→0.829)이고, 후속 질문은 멀티턴 쿼리 재작성으로 독립 검색어로 풀립니다. 답변 **생성**은 Ollama(Qwen2.5-14B-Instruct Q4_K_M, keep_alive=-1 상주 + 기동 워밍업)로 연결돼 멀티턴·SSE 스트리밍·면책 100% 보장까지 동작합니다. 웹앱(`web/`, Next.js 14 + TDS)은 LLM 채팅(👍/👎 피드백·금액 경고·근거 패널)·둘러보기·관계 그래프·다크모드·운영자 대시보드(`/admin`)·기능 플래그를 갖추고 PM2로 기동됩니다. 운영 중 품질을 끌어올리는 **P1.1~P1.5·P2.1~P2.5** 트랙은 P1.3 VLM 표복원만 남기고 완료됐습니다([12 품질강화](12-품질강화.md)).

### 남은 일 (다음 단계)

1. **검수** — 변환·자동초안 전건 `미검수` → 표본 대조 후 `검수완료` 승격(M5). 검수 큐(`review_queue.py`)·검수 도구(`review_tool.py`)·단건 재임베딩은 갖춰졌고, `검수완료`는 사람만 확정(자동 확정 금지). 규정 미분류 **28개 규정번호 배정**(인사규정·복무규정 등 핵심 규정 포함), 가이드/용어/ERP 자동초안 확정.
2. **변환 실패 2건 fallback** — 여비 QnA HWP(파서 타임아웃) · 예산운용가이드 이미지 PDF를 LibreOffice/OCR로 복원.
3. **별표 VLM 표복원(보류)** — 별표/별지는 1급 청크로 분리됐으나(별표 적중 0/4→4/4), 깨진 거대 표의 VLM(Qwen2.5-VL) 복원 트랙은 보류 중(모델 변경 안 함).
4. **외부 접속 안정화** — Cloudflare 엣지 설정(Rocket Loader 등 대시보드, 사용자 액션) · pm2 startup(부팅 자동시작) 1회.
5. **번들 경량화·컬러 토큰** — 웹앱 번들 경량화, KEI 메인 컬러 토큰 교체(미정).
6. **운영 루틴** — 재임베딩/백업/모니터링 루틴 정립(P5). 거부율·👍/👎·인기질문·콘텐츠 갭 대시보드(`/admin`)로 자기개선 루프(검수 큐·콘텐츠 로드맵 피드백)는 가동 중.
...
gantt
    ...
    Chroma kei_regs 약 4,418청크 :done, m2b, after m2a, 1w
    ...
    section 품질 강화 (P1.1~P2.5)
    P1.1~P1.5 평가·리랭커·재작성·검수큐 :done, q1, after m3d, 2w
    P2.1~P2.5 피드백·금액신뢰·하위청킹·ERP연결·대시보드 :done, q2, after q1, 2w
    P1.3 별표 VLM 표복원(보류)  :q3, after q2, 1w
...
> [!todo]
> 확인 필요: 각 마일스톤의 실제 캘린더 일정·담당 인원. 인원/구체 일정은 미정이므로 위 기간은 상대 순서 표현용 placeholder입니다(M0~M3 실측 완료 + M4 로컬 기동 완료 + 품질 강화 P1.1~P2.5 완료(VLM 보류 제외), `done` 표시).
...
flowchart TD
    ...
    M1 --> M2["M2 색인 ✅<br/>제N조/헤딩 청킹 + 별표 1급·하위청킹 + Chroma kei_regs (약 4,418)"]
    M2 --> M3["M3 통제형 RAG API ✅<br/>검색·리랭커·출처·인증·멀티턴·SSE 스트리밍 (Ollama)"]

    M2 --> Q["품질 강화 ✅<br/>P1.1~P1.5·P2.1~P2.5 (VLM 표복원만 보류)"]
    M3 --> Q
    ...
    M4b --> M4
    M4 --> M5["M5 검수·운영 안착 🔶<br/>검수 큐·피드백·대시보드 완료 / 전건 검수·Zero Trust 대기"]
    Q -. "피드백·거부율·인기질문 → 검수 큐" .-> M5
    ...
    class M0,M1,M2,M3,M4a,M4b,Q done;
```
...
- **[LLM]은 M3(통제형 API)에 의존.** 채팅은 그림이 아니라 텍스트+임베딩 검색으로 답하므로, 색인(M2)과 출처 강제 API(M3)가 선행돼야 합니다. 현재 검색(밀집 → 리랭커 재점수 → top-5)·멀티턴 쿼리 재작성·근거주입·출처에 더해 **답변 생성**(Ollama, Qwen2.5-14B-Instruct Q4_K_M)·인증·채팅기록·멀티턴·SSE 스트리밍·면책 100% 보장까지 검증됐습니다.
- **품질 강화 트랙은 M2·M3 위에서 운영 중 개선.** 평가 하베스트(`eval/`)·리랭커·멀티턴 재작성·별표 1급 청크·하위청킹·금액 신뢰 강화·ERP/서식 연결·피드백 루프·운영자 대시보드가 핵심 경로를 막지 않고 얹혀 동작합니다(P1.3 VLM 표복원만 보류). 상세는 [12 품질강화](12-품질강화.md).
- **검수(M5)는 콘텐츠 생성과 병행.** 변환·생성물은 검수 전까지 `검수상태: 미검수`를 유지하며, 검수는 M1 이후 상시 진행합니다. 검수 큐·도구·피드백 신호로 우선순위를 매기되, `검수완료` 확정은 사람만 합니다(자동 확정 금지).
...
- 도구: [`tools/02_chunk_and_embed.py`](../tools/02_chunk_and_embed.py). 실측 **약 4,418 청크**(별표/별지 1급 청크 분리 + 긴 청크 하위청킹 반영, 재색인 4345→4418). 규정은 첫 제N조 앞 머리말(규정명·제정/개정 이력·표)을 조=`""` 청크로 포함.
- 청킹: 규정은 **제N조 단위(조문 1개 = 청크 1개)**. 가이드·ERP·용어는 **헤딩 단위(`####`·`##`)**, 헤딩이 없으면 문단 패킹. 고정 길이 청킹 금지. 01이 넣은 H1 제목·변환 경고 콜아웃은 임베딩 전 제거(노이즈 감소).
- 별표/별지 1급 청크(P1.3): 별표/별지를 독립 청크로 분리(조=`별표 N`, `refs`=인용 조문, 토글 `CHUNK_BYEOLPYO`). 별표 적중 0/4→4/4. 깨진 거대 표의 VLM 복원은 보류.
- 긴 청크 하위청킹(P2.3): `max_seq_len`(2048) 초과 청크를 항(①②)→호(1./가.)→문단→줄 순으로 분할(`subsplit_long_chunks`, 토글 `CHUNK_SUBSPLIT`). 조 라벨(`제N조`)·출처·앵커·평가 strict 메타는 유지하고 하위 인덱스만 '부분'(2/3) 표기. 별표/별지(표)는 분할하지 않음.
- 임베딩: `nlpai-lab/KURE-v1`(XLM-RoBERTa·BGE-M3 계열, 컨텍스트 8192), `normalize_embeddings=True`, 양자화 안 함.
- 메모리: 큰 batch + 긴 조문은 CUDA OOM 유발 → **batch 8 + max_seq_len 2048**(+`expandable_segments`)로 해결. 2048 토큰 초과 청크는 하위청킹(P2.3)으로 분할 색인합니다.
...
- 검색 품질(P1.4·P1.5): 기본 밀집(KURE-v1)에 **리랭커**를 적용 — 밀집 top-20 → `BAAI/bge-reranker-v2-m3`(온프레미스 cross-encoder) 재점수 → top-5(`RAG_RERANK`, strict Hit@1 0.600→0.829·@5 1.000, 실패 시 밀집 강등). **멀티턴 쿼리 재작성**(`condense_query`, `RAG_QUERY_REWRITE` 기본 on)은 후속 질문을 직전 맥락으로 독립 검색어로 바꾸되 답변·근거는 불변(실패 시 원 질문 강등). 하이브리드(BM25+RRF, `bm25_index.py`)는 평가상 이득 없어 기본 off, opt-in 인프라만 보존.
- ERP·서식 연결(P2.4): 시스템(ERP) 근거 블록에 '(ERP 시스템)' 라벨 + SYSTEM 프롬프트가 메뉴·경로(화면ID 등)를 근거에 있을 때만 무환각으로 안내. 출처(`_src`)에 `type`을 실어 UI에 🖥 ERP/📄 서식 칩으로 표시.
...
- 표면: `/v1/chat/completions`(무상태, Open WebUI용) + `/app/*`(인증·채팅·멀티턴·메시지별 근거·👍/👎 피드백·통계). 멀티턴은 세션 메시지를 재생하되 근거는 매 턴 새로 검색하고, 응답은 SSE 스트리밍(`meta`→`delta`→`done`)으로 흘립니다.
- 가드레일 보장: [근거] 밖 내용(금액·한도·기한) 금지·"규정에서 확인되지 않습니다", 출처 `[규정명 제N조]`, 면책 문구를 `_ensure_disclaimer`로 100% 보장(0.806→1.000). 스트리밍 `answer_stream`도 동일.
- 참조: [05 RAG 설계](05-rag-design.md), [12 품질강화](12-품질강화.md), [ADR 0003](adr/0003-controlled-rag-api.md).
...
콘텐츠 신뢰성과 접근 통제, 운영 절차를 갖춥니다. 검수 도구·피드백 루프·운영자 대시보드(자기개선 루프)는 갖춰졌고, 전건 검수와 외부 접근 통제 안착이 남았습니다.

- 검수 워크플로: `검수상태: 미검수` → 사람이 원문 대조 후 `검수완료`로 승격. 가이드는 항상 `[[규정명#제N조]]` 위키링크로 원문 근거 표시. 현재 4개 섹션 **전건 미검수**(규정·가이드·용어·ERP 자동초안 포함)라 본격 검수는 아직 시작 전입니다. 검수 큐([`review_queue.py`](../tools/review_queue.py), 읽기전용 우선순위 점수=유형·별표·미분류·피인용+👎피드백)·검수 도구([`review_tool.py`](../tools/review_tool.py))·단건 재임베딩은 준비됐으며, ⛔`검수완료` 확정은 사람만 합니다(자동 확정 금지).
- 답변 피드백 루프(P2.1): 채팅 답변에 👍/👎(+사유), `Feedback` 테이블(사용자·메시지당 1건 upsert/toggle, 소유 격리). `feedback_export.py`→`tools/.feedback_signals.json`→검수 큐 우선순위 가산. ⛔검수상태 자동변경 없음.
- 금액 신뢰·기준일(P2.2): 금액/한도 답변 경고 + 근거 스니펫 수치 `<mark>` 강조 + 근거별 검수상태 배지(재임베딩 불필요). footer "📑 규정집 기준일"(`web/lib/site.ts` `CORPUS_AS_OF`, 규정 원문 적재일).
- 운영자 대시보드(P2.5): `/admin`에 활동·거부율·👍/👎·인기질문·콘텐츠 갭(`GET /app/stats`, 관리자 전용). 거부/👎/인기 질문 → 검수 큐·콘텐츠 로드맵으로 흐르는 자기개선 루프.
...
- 🔒 개인정보(P2.5): 서버사이드 RAG라 진짜 E2E 암호화는 불가(LLM이 평문 필요). 대신 ⓐ관리자도 타인 채팅을 읽는 엔드포인트 없음(소유자 검증) ⓑ`/stats`·`/feedback`은 질문·답변 본문 미반환(규정 메타·집계만) ⓒ인기질문/갭은 서로 다른 사용자 K명 이상(`STATS_MIN_USERS` 기본 3) k-익명 집계만.
- 운영: 색인 재빌드 주기, 변환 검수 큐, 백업 등.
- 참조: [07 보안·거버넌스](07-security-governance.md), [10 운영](10-operations.md), [09 기여 가이드](09-contributing.md), [12 품질강화](12-품질강화.md), [ADR 0005](adr/0005-on-prem-zero-trust.md).
...
| **별표 VLM 표복원** | 깨진 거대 표를 VLM(Qwen2.5-VL)으로 마크다운 재추출 | P1.3 후속 보류 트랙(모델 변경 안 함) |

> [!note]
> 이 백로그 항목은 핵심 경로(M0~M5)를 막지 않습니다. M5 안착 후 가치/비용을 보고 선별 도입합니다. (리랭커·멀티턴 쿼리 재작성은 P1.4·P1.5로, 평가 하베스트(`eval/`)는 P1.1로, 하이브리드 검색(BM25+RRF)은 opt-in 인프라로, SSE 스트리밍은 `/app/*` 멀티턴 응답으로 이미 구현돼 백로그에서 제외했습니다. 상세는 [12 품질강화](12-품질강화.md).)
