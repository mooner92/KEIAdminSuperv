...
| 인터넷 공개 금지 | [뇌] Quartz, [LLM] Open WebUI+vLLM **두 화면 모두** 공개 라우트를 갖지 않는다. | ⛔ 절대 규칙 5 |
...
| --- | --- | --- | --- | --- |
| 1. 망 경계 | Cloudflare Zero Trust Access | "누가 사이트에 도달할 수 있나" | 신원 기반 접근 정책 + Tunnel. 공개 인바운드 포트 없음 | [뇌]·[LLM] 둘 다 이 뒤에 위치 |
