...
> **v2(Spotify 리디자인) 반영됨**(2026-07-28 dev·main 병합) — 원본 명세 =
> [`../design/horong-v2/`](../design/horong-v2/). 핵심: **다크 기본 · 웜 차콜 3단 · 단색 앰버
> 1색 · 사이드바 2패널 셸(GNB/푸터 폐지) · 온디맨드 근거 패널 · 고밀도 트랙리스트 · 글자 커버
> 타일**. P3·P7은 개정본(§1)이 정본. 미채택: 칩 패싯(좌측 필터 유지) — `design/horong-v2/README.md` 결정 로그.
...
**P3. 가독성 우선 — v2에서 '고밀도 트랙리스트'로 개정.**
v1은 여백 우선이었으나 v2(Spotify)는 **행 52px 밀도 + 호버 톤**으로 스캔 속도를 우선한다.
목록 카드 상자를 없애고 행이 그라운드 위에 직접 놓인다. 한 행 = 한 문서, 한글 `keep-all`은 불변.
...
**P7. 그라데이션은 로고 전용(v2에서 강화).**
v2부터 그라데이션은 **HorongMark 로고(+랜딩 히어로 워드 — 소개 유지 결정)에만** 쓴다.
CTA·활성 상태·말풍선·아이콘 칩의 그라데이션은 전부 폐기 — **단색 앰버 1색**(`--color-primary`)이
기능색이다(다크 #ffc94d / 라이트 #e8a20c, 위 텍스트는 `--color-on-primary` 거의 검정).
...
| **2026-07-28** | **v2 Spotify 리디자인 착수**(`design/horong-v2/`, feat/spotify-v2) — 다크 기본·웜 차콜 3단·단색 앰버·사이드바 2패널 셸(GNB/푸터 폐지)·온디맨드 근거 패널·고밀도 트랙리스트·글자 커버 타일(이모지 폐기). P3·P7 개정. 소개/랜딩은 현행 구성 유지·좌측 필터 패널 유지(칩 패싯 미채택)(사용자 확정) |

> 변경 시 **이 문서를 먼저 갱신하고 코드에 반영한다**(P1 일관성). 값이 바뀌면 `globals.css`가 정본이고,
> 결정의 이유는 여기에 남긴다.


## 카드 경계 — `--color-surface-raised` (2026-08-03 신설)

실사용 지적("어디까지가 한 콘텐츠인지 모르겠다")에서 출발. 브라우저 계산값으로 확인한
원인: 카드 배경 `rgb(23,22,19)` = 부모 패널과 **같은 색** → 면 대비 0, 게다가 테두리는
`rgba(255,255,255,.07)` 헤어라인이라 선 대비도 없었다.

**규칙**: 목록형 카드는 다음 넷을 함께 쓴다.
1. 표면 `--color-surface-raised`(다크 `#211f1b` = 3단 심도의 세 번째 · 라이트 `#ffffff`)
2. 테두리 `--color-border-strong`(헤어라인 `--color-border`는 카드 경계에 쓰지 않는다)
3. `--shadow-card`
4. 좌측 3px 스트라이프(`--hr-grad-flame`, 도메인에 따라 심각도 색) — 카드 시작 앵커

**호버는 보조 신호일 뿐**이다 — 정적 상태에서 이미 구분돼야 한다(휴대폰엔 마우스가 없다).
⚠ 면 대비 요구는 **다크의 요구**다: 라이트는 흰 카드가 천장이라 여지가 없고 그림자가 분리를
담당한다(팔레트 주석 "다크에서 옅은 그림자는 안 보인다"의 대칭).
회귀 = `web/verify-card-contrast.mjs`(면 대비·선 대비·그림자·스트라이프·호버, 다크/라이트 각 5종).
