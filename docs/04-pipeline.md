...
> 4개 섹션 코퍼스(규정·가이드·용어·사내 시스템)가 [LLM] RAG 채팅 답변으로 흐르기까지의 변환·교차링크·임베딩 스크립트.
...
flowchart LR
    ...
    ESRC["사내 시스템 기능 문서\nERP·EIP·PMS 등"] --> S01D["01d_system_to_md.py\n사내 시스템 변환"]
...
| 시스템 변환 | `01d_system_to_md.py` | 사내 시스템 기능 문서(ERP·EIP·PMS 등) | `40_시스템/` (system, 7개 시스템 65노트) |
...
| 조문 참조그래프 (Track A) | `01i_clause_xref.py` | 규정 원문(`vault_parse.py`) | `tools/index/clause_xref.json`(준용·인용·별표) |
| 정의어 사전 (Track A) | `01j_defterms.py` | 규정 원문 | `tools/index/defterms.json`(정의 + 교차충돌) |
...
> [!note] 다중 시스템으로 일반화됨
> `01d_erp_to_md.py`(ERP 전용)는 이후 **`01d_system_to_md.py`**(같은 3단 구조 + `--bundle` 모드)로 일반화됐다. 이 스크립트가 ERP 외 6개 시스템(통합정보시스템(EIP)·연구관리시스템(PMS)·웹메일·그룹웨어·웹디스크·전자도서관)까지 `40_시스템/`에 적재해, 현재 **7개 시스템 · 65개 노트**가 있다(ERP 상세가이드 7 + **PMS 상세가이드 11**(`01v_pms_deep_to_md.py`, 도움말 PDF·화면캡처 판독본 → 탭별 화면 상세) 포함). 둘러보기 섹션 라벨은 **'사내 시스템'**이다.
...
| 출력 | `<vault>/40_시스템/<모듈>.md` (type:system, ERP 12개 모듈) |
...
> [!note] 01e도 다중 시스템으로 일반화됨
> `01e_erp_crosslink.py`(ERP 전용)는 **`01e_system_crosslink.py`**로 일반화돼, 7개 시스템 노트를 각 분류(frontmatter)별 도메인 키워드로 관련 규정과 잇는다(멱등, `<!-- system-crosslink -->` 마커).
...
`chunk_label()`이 규정 청크 머리를 보고 `(kind, 라벨)`을 판정한다 — 제N조면 `조`에 `제N조`, 줄머리 `[별표 N]`/`[별지 제N호]`면 각각 `별표 N`/`별지 제N호`를 채우고 `별표="Y"`로 표식한다(가이드·시스템·용어는 `조` 빈값). `_templates`가 경로에 포함된 파일은 건너뛴다.
...
### 목적
임베딩 검색으로 관련 청크(규정 제N조·가이드/시스템/용어 헤딩) top-k를 회수해 근거로 주입하고, 로컬 LLM(Ollama)으로 답한 뒤 회수 청크를 출력한다. 운영 전 검증·디버깅용 CLI다. `--retrieve-only`로 **LLM 없이 검색만** 돌려 회수 품질을 따로 확인할 수 있다.
...
LLM_BASE    = "http://127.0.0.1:11436/v1"    # 격리 Ollama v0.31.1 (OpenAI 호환)
LLM_MODEL   = "hf.co/unsloth/Qwen3.5-9B-GGUF:Q4_K_M"  # Ollama에 올라간 모델 태그
```

1. 질문을 `KURE-v1`로 임베딩(`normalize_embeddings=True`) → `col.query(n_results=k)`.
2. 회수된 각 청크를 `[규정명 제N조]\n본문` 블록(가이드·시스템·용어는 `[제목/용어]`)으로 만들고 `\n\n---\n\n`로 연결해 컨텍스트 구성.
...
LLM은 일반 instruct 모델을 **격리 Ollama v0.31.1**(127.0.0.1:11436, OpenAI 호환)로 서빙한다(`Qwen3.5-9B` GGUF Q4_K_M, 코더/VL 아님). 사고모드는 끈다(`reasoning_effort=none`). `keep_alive=-1`로 모델을 상주시키고 기동 시 워밍업해 첫 질문 콜드스타트를 없앤다. 한국어 특화 대안은 EXAONE/Kanana.
...
- `LLM_MODEL`/`LLM_BASE`: 환경변수로 오버라이드 — 격리 Ollama v0.31.1에 떠 있는 모델명·주소와 일치시킬 것.
...
백엔드(임베딩 `KURE-v1`·Chroma `kei_regs`·Ollama 클라이언트)는 **지연 로딩**이라 `/v1/models` 같은 모델 등록 요청에는 즉시 응답한다. `retrieve(query, k=5)`가 03과 동일하게 `[규정명 제N조]`(가이드·시스템·용어는 `[제목/용어]`) 블록 컨텍스트를 만든다. 환경변수 `CHROMA_DIR / RAG_COLLECTION / EMBED_MODEL / LLM_BASE / LLM_MODEL / RAG_MODEL_ID / RAG_TOPK`로 설정을 오버라이드한다.
...
sequenceDiagram
    ...
    participant L as 격리 Ollama v0.31.1 (Qwen3.5-9B GGUF Q4_K_M)
    U->>W: 질문(멀티턴)
    W->>A: POST /app (세션)
...
### 알려진 한계
> [!warning]
> - 답변 생성은 **격리 Ollama v0.31.1**(`Qwen3.5-9B` GGUF Q4_K_M, 127.0.0.1:11436/v1, PM2 `kei-ollama-v031`, ctx 8K)가 떠 있어야 한다. `keep_alive=-1` 상주 + 기동 워밍업으로 첫 질문 콜드스타트를 없앴다. NVIDIA 드라이버 535라 CUDA 대신 Vulkan로 GPU를 쓴다(550+ 업그레이드 시 자동 CUDA 전환). 임베딩(KURE-v1)은 1장으로 충분(실측).
...
# 2) 청킹(규정 제N조 / 가이드·시스템·용어 헤딩) + KURE-v1 임베딩 → Chroma(kei_regs) — 기본 클린 리빌드
...
#   → 답변 생성: 격리 Ollama v0.31.1(127.0.0.1:11436, Qwen3.5-9B GGUF Q4_K_M) 필요
...
- **가이드 65개**(01c, HWP/HWPX/PDF/PPTX). 스캔 이미지 PDF 1건(예산운용가이드)은 `image-pdf` 플레이스홀더로 OCR 폴백 대상.
- **사내 시스템 33개 노트**(01d_system_to_md.py, 7개 시스템 = ERP·EIP·PMS·웹메일·그룹웨어·웹디스크·전자도서관), **용어 84개**(01f). 전부 자동초안.
...
| 연구행정 가이드 | `10_업무가이드/` | guide | 65 |
| 용어집 | `30_용어집/` | term | 84 |
| 사내 시스템 | `40_시스템/` | system | 33 |
...
- 시스템↔규정(01e_system_crosslink)·용어↔시스템/규정(01g) 키워드 매칭 + 규정 상호참조(01b) `[[ ]]` 엣지 생성. 그래프(`web/graph`)에서 코퍼스 문서 수만큼 노드(**293 노드**)가 생기고, 교차링크 덕분에 시스템 모듈이 허브가 된다.

### 02 청킹·임베딩

- **약 4,545 청크**(별표/별지 1급 청크 분리 + 긴 조문 하위청킹 반영, 사내 시스템 7종 적재 후). 대다수가 regulation 조문 청크, 나머지는 guide·system·term 헤딩 청크.
...
- 청킹 단위: 규정 제N조(+ 문서당 머리말 1청크) + **별표/별지 1급 청크**(P1.3) / 가이드·시스템·용어는 헤딩(`####`·`##`).
...
- **검색·근거 주입·출처는 검증됨.** 답변 생성은 **격리 Ollama v0.31.1(Qwen3.5-9B GGUF Q4_K_M)** 로 동작하며 SSE 스트리밍(`meta→delta→done`)·멀티턴(매 턴 새 검색)까지 구현됐다. `keep_alive=-1` 상주 + 워밍업으로 첫 질문 콜드스타트 제거.
