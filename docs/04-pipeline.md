...
> [!note] 다중 시스템으로 일반화됨
> `01d_erp_to_md.py`(ERP 전용)는 이후 **`01d_system_to_md.py`**(같은 3단 구조 + `--bundle` 모드)로 일반화됐다. 이 스크립트가 ERP 외 6개 시스템(통합정보시스템(EIP)·연구관리시스템(PMS)·웹메일·그룹웨어·웹디스크·전자도서관)까지 `40_시스템/`에 적재해, 현재 **7개 시스템 · 33개 노트**가 있다. 둘러보기 섹션 라벨은 **'사내 시스템'**이다.
