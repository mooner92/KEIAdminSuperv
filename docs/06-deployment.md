...
> 하나의 볼트, 두 개의 화면을 운영 서버에 올린다. [뇌] Next.js + TDS 정적 사이트와 [비서] Open WebUI + vLLM을 같은 마크다운 볼트 위에서 서빙하고, 둘 다 Cloudflare Zero Trust 뒤에 둔다.
...
| Next dev ([뇌]) | `cd web && npm run dev` | `3100` | 로컬 미리보기 전용. `http://127.0.0.1:3100` |

> [!note] [뇌] 운영은 정적 export
> Next dev(`3100`)는 검수용 미리보기 전용이다. 운영의 [뇌]는 dev 서버가 아니라 `npm run build`의 `web/out/`을 nginx(`127.0.0.1`)로 서빙하므로 별도 포트를 점유하지 않는다. 서버 런타임이 필요 없다.

---

## 3. [뇌] Next.js + TDS 배포

[뇌] 화면(사람이 탐색하는 목록·문서·관계 그래프)은 **Next.js 14 + Toss Design System(TDS)** 으로 만든다. 코드는 레포의 [`../web/`](../web/) 디렉터리에 있다. Next.js 정적 export로 빌드해 `web/out/`을 nginx(`127.0.0.1`)로 서빙하고, 기존 [비서]와 같은 Cloudflare Zero Trust 뒤에 둔다 — 서버 런타임이 필요 없다.
...
- **서빙:** Next.js 정적 export([`../web/next.config.mjs`](../web/next.config.mjs)의 `output: "export"`) → `web/out/` → nginx `127.0.0.1` → Cloudflare Zero Trust(사내 전용). 서버 런타임 불필요.
...
- **볼트 소비:** [`../web/lib/vault.ts`](../web/lib/vault.ts)가 볼트(`KEI-행정가이드/`, git 비추적·Syncthing 동기화)를 **빌드타임 read-only**로 읽는다. `VAULT_DIR` 환경변수로 볼트 경로를 지정하며 기본값은 레포 루트다. `web/node_modules`·`.next`·`out`은 `.gitignore`.
...
VAULT_DIR=<볼트경로> npm run build       # → web/out/ 정적 export 산출물 생성
```

산출된 `web/out/`을 nginx 루트로 지정하고, 기존 Cloudflare Tunnel에 이 사이트로 향하는 라우트를 추가한다.
...
sequenceDiagram
    ...
    participant O as web/out/
    participant N as nginx (127.0.0.1)
    participant T as Cloudflare Tunnel

    V->>W: VAULT_DIR (빌드타임 read-only)
    W->>B: 목록·문서·그래프 데이터
    B->>O: npm run build (output: export)
    O->>N: nginx 루트 = web/out/
    N->>T: Tunnel 라우트 추가
...
단일 앱·단일 볼트 안에서 섹션(규정집 / 연구행정 가이드 / 용어집)을 분리한다. 가이드는 볼트의 `10_업무가이드/`에 문서를 추가하면 자동으로 합류한다.

- **목록:** TDS `SearchField`·`SegmentedControl`로 검색/섹션 탭, 가독성 위주의 행.
- **문서:** 메타 칩 · 본문 · 백링크 · `제N조` 앵커로 조 단위 점프. 위키링크 `[[ ]]`(규정 상호참조)는 내부 라우트로 연결되고, 이름 변이(공백·가운뎃점 `·`·`.`·`및`)도 정규화로 자동 흡수한다.
- **관계 그래프:** `react-force-graph-2d`(노드 클릭 → 문서 이동, 코드 스플릿).

> [!note] 실측(2026-06-19)
> `npm run build` 성공, 정적 115페이지(목록·관계 그래프 + 문서 111). 한글 mojibake 0, 위키링크 내부 네비 + `제N조` 앵커 동작, 그래프 111 노드·82 연결, TDS 컬러 적용. 디자인 원칙·토큰·컴포넌트 규약은 [design-system.md](design-system.md) 참조.
...
flowchart LR
    ...
    CF --> NSITE["Next.js + TDS 사이트<br/>(web/out/ · nginx 127.0.0.1)"]
...
- **[뇌] Next.js + TDS:** 볼트 갱신 → `cd web && npm run build`(`VAULT_DIR`) → `web/out/` 교체 → nginx 반영.
...
| 7 | [뇌] `cd web && npm run build` → `web/out/` → nginx (127.0.0.1) | §3 |
