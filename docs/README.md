...
| 의견수렴·유지보수 자동화 | ✅ 구현·검증 | `/feedback` 능동 제보(오류·누락·개선, 진입점: 푸터·문서 드로어·허브) + /admin 📮 의견함(상태 처리·🔔 알림). 매시간 로컬 LLM(`feedback_analyze.py`, PM2 cron)이 접수 제보를 그룹·중복 제거해 로컬조치/코드작업 계획안 생성 — 없으면 '없음' 기록만 ([51](51-의견수렴-유지보수자동화.md)) |
...
| 운영 루프 | [13 기능 플래그](13-feature-flags.md) · [14 피드백 루프](14-feedback-loop.md) · [51 의견수렴·유지보수 자동화](51-의견수렴-유지보수자동화.md) · [52 오토픽스·컨테이너 로드맵(계획)](52-오토픽스-컨테이너-로드맵.md) · [20 코퍼스 관리](20-코퍼스-관리-설계.md) · [21 관리자 UX](21-관리자-UX-개편.md) · [33 아카이브 이전(보류)](33-아카이브-이전-계획.md) |
...
| 이력·거버넌스 | [요구사항 처리 대장](요구사항-처리대장.md)(요구사항→처리·검증·상태 지도) · [adr/](adr/)(아키텍처 결정) · 서버 이전은 `manual/`(gitignore) |
