...
핵심 구조는 **하나의 볼트, 두 개의 화면**입니다. 단일 진실원천(Source of Truth)인 마크다운 볼트 `KEI-행정가이드/`를 두 화면이 공유합니다. 현재 볼트는 **4개 섹션**(규정집 111 · 연구행정 가이드 65 · 용어집 84 · 사내 시스템 7종: ERP·통합정보시스템(EIP)·연구관리시스템(PMS)·웹메일·그룹웨어·웹디스크·전자도서관)으로 구성됩니다.
...
| 영역 | 상태 | 요약 |
|------|------|------|
| 코퍼스 | ✅ 4섹션 | 규정집 111 · 연구행정 가이드 65 · 용어집 84 · 사내 시스템 7종(ERP·EIP·PMS·웹메일·그룹웨어·웹디스크·전자도서관). 임베딩 청크 약 4,545(긴 조문 하위분할·별표 1급 청크·전사 시스템 적재 반영). 전건 '검수상태: 미검수' |
...
| [뇌] 웹앱 | ✅ LLM·둘러보기·관계 그래프 | 그래프 노드·연결(4색), 교차링크로 사내 시스템 모듈이 허브. 다크모드/테마(라이트·다크·시스템) |
| 운영 버전 | ✅ 운영/개발 분리 | 운영(prod) 3100/9000(`feat/0620`), 개발(dev) 3101/9001(`feat/0703` git worktree, chroma·app.db·볼트·세션키 완전 격리) |
