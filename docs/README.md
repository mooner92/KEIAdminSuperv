...
| 의견수렴·유지보수 자동화 | ✅ 구현·검증 | `/feedback` 능동 제보 + /admin 📮 의견함(섹션·상단 페이저·🔔). 분석은 **제보 제출 이벤트+매시 백스톱+지금 분석** 3트리거, **게이트(0~3) 보고서**(코드 조사·RAG 대조·패치노트 재발 대조·Claude 프롬프트·확인포인트) ([51](51-의견수렴-유지보수자동화.md)) |
| 오토픽스 Phase A | ✅ 구현(실전 E2E는 버튼 대기) | 의견함 🤖 → 격리 worktree에서 무인 Claude Code(구독 인증·Bash 미부여) → 관문 4종 → `autofix/` 브랜치+compare URL 🔔. ⛔라이브 무접촉·머지는 사람. 회귀 11/11 ([52](52-오토픽스-컨테이너-로드맵.md) §9) |
...
| 운영 루프 | [13 기능 플래그](13-feature-flags.md) · [14 피드백 루프](14-feedback-loop.md) · [51 의견수렴·유지보수 자동화](51-의견수렴-유지보수자동화.md) · [52 오토픽스·컨테이너 로드맵(A 구현·B/C 계획)](52-오토픽스-컨테이너-로드맵.md) · [20 코퍼스 관리](20-코퍼스-관리-설계.md) · [21 관리자 UX](21-관리자-UX-개편.md) · [33 아카이브 이전(보류)](33-아카이브-이전-계획.md) |
...
| 보안 | [44 보안 명세 — 로그인 게이트·조치 매트릭스·공개 전 체크리스트](44-보안-로그인게이트.md) · [63 보안 스캔 1차 대응](63-보안-스캔-대응.md)(게이트 우회·인증 3종·가드 fail-open·심각도 실측 재평가) · [65 보안 스캔 2차](65-보안-스캔-2차.md)(22→8건 · ⚠1차 조치 불완전 F1 · 디렉터리 권한 구멍) |
| 이력·거버넌스 | [요구사항 처리 대장](요구사항-처리대장.md)(요구사항→처리·검증·상태 지도) · [53 문서관리 규약](53-문서관리-규약.md)(필수 갱신 매트릭스·생성 규칙) · [adr/](adr/)(아키텍처 결정) · 서버 이전은 `manual/`(gitignore) |
