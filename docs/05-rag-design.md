...
flowchart LR
  ...
  RR --> CTX["컨텍스트 조립<br/>[규정명 조] 블록<br/>(시스템 근거엔 실제 시스템명 라벨)<br/>--- 구분"]
  CTX --> LLM["Ollama v0.31.1<br/>Qwen3.5-9B GGUF Q4_K_M<br/>temp=0.1"]
...
> **검색·근거주입·출처 표기에 더해 생성(LLM) 단계까지 엔드투엔드로 가동·검증됐다.** 생성 LLM은 **격리 Ollama v0.31.1**(OpenAI 호환, `127.0.0.1:11436/v1`, PM2 `kei-ollama-v031`, ctx 8K)로 **Qwen3.5-9B GGUF Q4_K_M**(unsloth, 약 5.7GB, apache-2.0)를 띄워 한국어 답변을 확인했다(예: "법인카드로 주말에 비품 사도 되나요?" → `법인카드관리및사용규칙` 근거 답변). 임베딩 KURE-v1은 변경 없이 1장으로 충분하다(실측). 그 위에 운영 품질 작업이 적용됐다 — **리랭커(P1.4·§4.5), 멀티턴 쿼리 재작성(P1.5·§4.6), 사내 시스템·서식 연결(P2.4·§4.7), 면책 문구 100% 보장(§5), SSE 스트리밍(§7b.4)**. 03은 여전히 `--retrieve-only`(LLM 없이 검색만), 04는 LLM 미연결 시 회수 출처만 반환하는 그레이스풀 동작을 제공한다(§4.4·§7). GPU는 2×Quadro RTX 6000 24GB이며 공유·변동적이라 모델 배치 전 `nvidia-smi`로 확인한다(리랭커·재색인은 여유 GPU — 주로 cuda:1 사용). Qwen3.5-9B은 Q4_K_M GGUF(약 5.7GB)라 단일 24GB GPU에 넉넉히 상주한다. NVIDIA 드라이버 535 환경이라 격리 Ollama v0.31.1은 CUDA 대신 Vulkan으로 GPU를 쓰며(550+ 업그레이드 시 자동 CUDA 전환), vLLM은 채택하지 않는다.
...
| 컬렉션명 | `kei_regs` |
| 적재 규모(실측) | **5,542청크** — 5섹션·374문서(규정집 111 · 가이드 65 · 용어집 119 · **사내 시스템 65** · 대외업무 14 — ERP·통합정보시스템(EIP)·연구관리시스템(PMS)·웹메일·그룹웨어·웹디스크·전자도서관 7개 시스템. ERP/PMS는 **상세가이드 tier**(화면별 필드·버튼·업무흐름) 별도 보유, 2026-07-20 실측). 별표/별지 1급 청크(P1.3)·긴 조문 하위청킹(P2.3) 반영분 포함 |
...
| `type` | `regulation` / `guide` / `system`(사내 시스템) / `term` | 전부. UI에서 🖥 시스템(실제 시스템명)/📄 서식 칩 구분에 사용(§4.7) |
| `path` | 볼트 내 원본 노트의 **상대경로** | 전부 |

> [!note] 청킹 단위
> `type=regulation`은 **제N조 = 청크 1개**(고정 길이 청킹 금지). 첫 `제N조` 앞 머리말(규정명·제정/개정 이력·표)은 `조=""` 머리말 청크로 별도 보존한다(문서당 1개). `guide`·`system`(사내 시스템)은 **헤딩(####/##) 단위**, `term`은 노트 전체가 1청크다. **별표/별지는 1급 청크로 분리**(조=`별표 N`, P1.3), **`max_seq_len`(2048) 초과 청크는 항·호·줄 단위로 하위분할**(조 라벨·메타 유지, P2.3)한다. `_templates`는 청킹에서 제외한다. 상세는 [04-pipeline.md](04-pipeline.md)·[12-품질강화.md](12-품질강화.md), 결정 근거는 [ADR 0002](adr/0002-article-level-chunking.md).
...
> 위 표는 **회수(retrieval)의 정확성**을 보여준다. 회수된 조문 본문을 근거로 한 **실제 답변 생성**도 현재 Ollama v0.31.1(Qwen3.5-9B GGUF Q4_K_M)로 가동돼 엔드투엔드로 확인됐다(§1·§7). 다만 회수된 조문은 모두 `검수상태=미검수`이므로, 운영 답변 근거로 쓰기 전 사람 검수가 선행돼야 한다.
...
03의 LLM 경로(플래그 미지정 시)는 환경변수 `VLLM_BASE`(현재는 격리 Ollama v0.31.1의 `127.0.0.1:11436/v1`을 가리킴)·`LLM_MODEL`로 엔드포인트·모델을 오버라이드할 수 있고, LLM 호출이 실패하면 친절한 안내(엔드포인트 확인 / `--retrieve-only` 권유)를 출력한다.
...
### 4.7 사내 시스템·서식 연결 (P2.4)

규정만으로는 "그래서 어느 시스템에서 처리하냐"가 안 풀린다. 사내 시스템(`type=system` — ERP·통합정보시스템(EIP)·연구관리시스템(PMS)·웹메일·그룹웨어·웹디스크·전자도서관 7개) 청크가 근거로 회수되면, 컨텍스트 블록에 **실제 시스템명 라벨**을 붙이고(예: `[○○ 모듈 (ERP 시스템)]`·`[○○ (전자결재)]`), 시스템 프롬프트가 거기 적힌 **메뉴·처리 경로(화면ID 등)를 답변에 함께 안내**하게 한다 — 단 **근거에 있을 때만**(없는 경로·서식명은 지어내지 않음, 가드레일 유지).

- 구조화 출처(`_src`)에 `type`을 실어 UI가 **🖥 시스템(실제 시스템명) / 📄 서식** 칩으로 구분 표시한다(§8.3·§3.2).
...
6) [근거]에 '(… 시스템)' 항목(ERP·전자결재·대외업무·웹디스크 등)이 있으면, 그 시스템명과 메뉴·처리 경로를
   답변에 함께 안내한다 (근거에 없는 시스템·경로·서식명은 지어내지 않는다).
```
...
| 6 | `(… 시스템)` 근거(ERP·전자결재 등)가 있으면 메뉴·경로를 안내(없는 건 금지) | **규정→실제 처리 연결(P2.4·§4.7).** 시스템 경로 안내도 환각 금지 원칙 안에서만 |
...
> 생성 LLM은 마무리 면책 문구("최종 판단은…")를 종종 빠뜨린다(구 Qwen2.5-14B 평가셋 측정 ~19% 누락, 0.806). 그래서 모델 출력에 면책 어구가 없으면 [`../tools/rag_core.py`](../tools/rag_core.py)의 `_ensure_disclaimer()`가 **결정적으로 덧붙여 100% 보장**한다(0.806→1.000 by construction). 표현이 살짝 달라도 중복으로 두 번 붙지 않게 핵심 어구로 감지한다. **비스트리밍(`answer`)·스트리밍(`answer_stream`) 모두 동일**하게 적용된다(§7b.4). 이는 가드레일 약화가 아니라 강화다.
...
sequenceDiagram
  ...
  participant V as Ollama v0.31.1<br/>Qwen3.5-9B GGUF Q4_K_M
  U->>W: 질문
...
04의 동작은 환경변수로 조정한다: `CHROMA_DIR`, `RAG_COLLECTION`, `EMBED_MODEL`, `VLLM_BASE`(현재 격리 Ollama v0.31.1 `127.0.0.1:11436/v1`), `LLM_MODEL`(현재 `hf.co/unsloth/Qwen3.5-9B-GGUF:Q4_K_M`), `RAG_MODEL_ID`, `RAG_TOPK`. (`EMBED_MODEL`은 색인(02)·CLI(03)와 **반드시 동일** — §2.1 불변식.)
...
> 검색·근거주입·`x_sources`/`x_retrieved` 출처 반환은 물론 **실제 답변 생성까지 가동·검증됐다.** 현재 생성 LLM은 **격리 Ollama v0.31.1**(OpenAI 호환, `127.0.0.1:11436/v1`, PM2 `kei-ollama-v031`, ctx 8K)로 **Qwen3.5-9B GGUF Q4_K_M**(unsloth, 약 5.7GB, apache-2.0)를 띄워 사용하며, keep_alive=-1로 GPU에 상주하고 GPU0이 비어 있어 상주에 여유가 있다. 한국어 답변을 엔드투엔드로 확인했다(예: "법인카드로 주말에 비품 사도 되나요?" → `법인카드관리및사용규칙 제4조` 근거 답변). Qwen3.5-9B Q4_K_M(약 5.7GB)은 RTX 6000 단일 24GB에 넉넉히 올라가고, NVIDIA 드라이버 535라 격리 Ollama v0.31.1은 CUDA 대신 Vulkan으로 GPU를 쓴다(550+ 업그레이드 시 자동 CUDA 전환). vLLM은 채택하지 않는다(공유 Ollama `11434`는 qwen3_5 미지원이라 미사용 — 건드리지 않는다). 운영 정답성·출처 정확도·거부율은 §8 지표로 본측정 예정이다.
...
`04_rag_api.py`의 응답에는 **어떤 조문이 회수됐는지** 그대로 남기는 출처 필드가 들어간다(생성 LLM 미연결 시에도 회수 출처는 반환 — §7). 프론트 근거 패널이 소비하는 `x_sources`(구조화 출처: 규정명/조/분류/`type`/snippet/distance, 리랭커 켜진 경우 `rerank` 점수 포함)와, 디버그·하위호환용 `x_retrieved`(태그 문자열) 두 형태로 제공한다. `type`으로 UI가 🖥 시스템(실제 시스템명)/📄 서식 칩을 구분한다(§4.7). `/app` 채팅 경로에서는 같은 회수 근거를 답변 메시지의 `message.sources_json`에 함께 저장하므로, 지난 답변별로 "그때 무엇이 회수됐는지"를 사후에도 되짚을 수 있다(§7b.4). CLI(`03_rag_query.py`)는 `--retrieve-only`로 LLM 없이 회수된 조 목록·거리를 출력한다(§4.4). 이미 §4.3에서 6개 대표 질의의 회수 정확성을 이 경로로 검증했다.
...
| 사내 시스템·서식 연결 | ✅ **적용**(P2.4·§4.7). 실제 시스템명 라벨(ERP·전자결재 등, 7개 시스템) + 메뉴·경로 안내 + 🖥/📄 칩 |
