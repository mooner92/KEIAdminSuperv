...
[LLM] Open WebUI + vLLM 화면은 그림이 아니라 **텍스트 + 임베딩 검색**으로 답합니다. 사용자가 "이 업무 어떻게 처리하지?"라고 물으면, RAG API가 볼트의 규정 조각을 검색해 컨텍스트로 주입하고, 답변 끝에 `[규정명 제N조]` 형식으로 출처를 단 뒤 "최종 판단은 원문과 담당 부서 확인 바랍니다."를 덧붙입니다.
...
- **비표준 조문 표기.** `제 1 조`(공백)·`제1조의2`(가지번호)는 **2026-07-04 정규식 보강으로 정상 포착**(라벨 붕괴 해소 — `chunk_label`·`article_no`·`_jo_key`를 `제N조(?:의M)?`로 확장, 재임베딩 시 의N 라벨 62개). **부칙(附則)** 등 나머지 변형은 여전히 별도 대응 필요.
...
> [!done]
> ✅ **가지번호(`제N조의M`)·공백 변형(`제 N 조`) — 2026-07-04 정규식 보강 완료**(dev feat/0703). 제19조·제19조의2·제19조의3·제19조의4 구별, graph-expand `_jo_key`도 동일 반영.
> 남은 것: **부칙(附則) 청킹**(현재 극소수 파일만 부칙 포착 — 버전 정합성 감사 참조).
