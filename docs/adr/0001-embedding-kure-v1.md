...
- **온프레미스 GPU(A40) 구동.** 모델·임베딩 전부 사내에서 돌린다. 외부 임베딩 API는 사용하지 않는다.
...
| 실행 위치 | 사내 GPU(A40) | SentenceTransformer가 GPU 자동 사용 |
...
회수 품질이 답변 품질의 상한이고, A40 GPU에서 full precision으로 충분히 구동 가능하므로,
...
- **메모리:** 양자화하지 않으므로 모델이 GPU 메모리를 full precision으로 점유한다. A40에서 임베딩 모델 + vLLM(예: `Qwen/Qwen2.5-14B-Instruct`) + (필요 시) 표 재추출용 `Qwen2.5-VL`이 같은 GPU 자원을 두고 경합할 수 있다.
...
> [!todo]
> 확인 필요: A40의 정확한 **수량/총 VRAM**, 그리고 임베딩 모델 + vLLM(+ VLM) 동시 상주 시 메모리 예산. 서버 호스트명은 예시로 `data05lx`만 확인됨 — 그 외 호스트/IP는 미확정.
