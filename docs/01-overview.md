...
|---|---|---|---|
| **신입** | 행정 업무 자체가 처음 | "이 일을 **무슨 순서로** 하지?" — 용어부터 막힘 | [비서]가 업무 언어를 받아 멀티턴·스트리밍으로 쉬운 단계 설명 + 메시지별 `[규정명 제N조]` 근거 |
| **전입자** | 행정 경험은 있으나 KEI 규정 체계는 낯섦 | "**우리 기관에서는** 이걸 어느 규정이 정하나?" | [뇌] 관계 그래프로 규정·용어·ERP 관계 탐색, [비서]로 근거 확인 |
| **담당 부서** | 같은 질문을 반복해서 받음 | 반복 문의 감소, 답변의 **출처 일관성** | 1차 응대를 시스템이 흡수, 가이드·원문을 검수·관리하는 주체 |

> [!tip]
> 신입·전입자는 주로 **[비서]**(로그인 후 멀티턴 RAG 채팅, 질문→답변)로 들어오고, 담당 부서는 **[뇌]**(둘러보기·관계 그래프로 탐색·검수)와 볼트 작성으로 들어온다. 두 입구가 **같은 마크다운 볼트**를 본다. [뇌]·[비서]는 모두 Next.js 14 + Toss Design System 웹앱(`web/`)의 화면이며, [비서]는 Ollama 기반 로컬 LLM으로 답한다.

---

## 3. 솔루션 한 줄 + 가치 제안

> **사내 규정·가이드·용어·ERP를 단일 마크다운 볼트(4개 섹션·271문서)로 모으고, 같은 볼트를 두 개의 화면 — 탐색용 그래프 [뇌]와 출처 달린 답변용 비서 [비서] — 으로 제공하는 온프레미스 지식베이스 + 로컬 LLM 비서.**
...
flowchart TD
    ...
    V --> Assist["🤖 [비서] 멀티턴 RAG 채팅 + Ollama<br/>제N조 검색 + 근거주입(스트리밍)<br/>[규정명 제N조] 출처<br/>(행정 초보가 사용)"]
...
- **두 화면 모두 사내 전용.** [뇌] 둘러보기·관계 그래프, [비서] 채팅 모두 Cloudflare Zero Trust(이메일 인증) 뒤에 둔다. 인터넷 공개 금지.
...
flowchart LR
    subgraph net["사내망 · 인터넷 공개 금지"]
        ...
        ZT --> Web["🧠🤖 web/ Next.js+TDS<br/>둘러보기·관계 그래프·비서 채팅"]
