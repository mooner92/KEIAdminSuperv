...
> KEI(한국환경연구원) **행정 가이드 / 행정 LLM** 프로젝트의 출발점.
...
|---|---|---|---|
| **신입** | 행정 업무 자체가 처음 | "이 일을 **무슨 순서로** 하지?" — 용어부터 막힘 | [LLM]이 업무 언어를 받아 멀티턴·스트리밍으로 쉬운 단계 설명 + 메시지별 `[규정명 제N조]` 근거 |
| **전입자** | 행정 경험은 있으나 KEI 규정 체계는 낯섦 | "**우리 기관에서는** 이걸 어느 규정이 정하나?" | [뇌] 관계 그래프로 규정·용어·사내 시스템 관계 탐색, [LLM]으로 근거 확인 |
| **담당 부서** | 같은 질문을 반복해서 받음 | 반복 문의 감소, 답변의 **출처 일관성** | 1차 응대를 시스템이 흡수, 가이드·원문을 검수·관리하는 주체 |

> [!tip]
> 신입·전입자는 주로 **[LLM]**(로그인 후 멀티턴 RAG 채팅, 질문→답변)로 들어오고, 담당 부서는 **[뇌]**(둘러보기·관계 그래프로 탐색·검수)와 볼트 작성으로 들어온다. 두 입구가 **같은 마크다운 볼트**를 본다. [뇌]·[LLM]은 모두 Next.js 14 + Toss Design System 웹앱(`web/`)의 화면이며, [LLM]은 Ollama 기반 로컬 LLM으로 답한다.

---

## 3. 솔루션 한 줄 + 가치 제안

> **사내 규정·가이드·용어·사내 시스템을 단일 마크다운 볼트(4개 섹션·293문서)로 모으고, 같은 볼트를 두 개의 화면 — 탐색용 그래프 [뇌]와 출처 달린 답변용 LLM [LLM] — 으로 제공하는 온프레미스 지식베이스 + 로컬 LLM.**

코퍼스는 네 개 섹션으로 이루어진다: **규정집**(`20_규정원문/`, 111문서) · **연구행정 가이드**(`10_업무가이드/`, 65문서) · **용어집**(`30_용어집/`, 84문서) · **사내 시스템**(`40_시스템/`, 33문서 — ERP(행정관리)·통합정보시스템(EIP)·연구관리시스템(PMS)·웹메일·그룹웨어·웹디스크·전자도서관) = 총 **293문서**. 섹션 간 교차링크(특히 ERP 모듈↔규정·용어)로 그래프 엣지를 만든다.

가치 제안:

- **근거 있는 답.** 모든 답변 끝에 `[규정명 제N조]` 출처가 붙고, 근거에 없는 내용은 지어내지 않는다. 멀티턴 대화에서도 근거는 매 턴 새로 검색된다.
- **업무 언어로 물어도 됨.** 임베딩 검색이 "여비 청구" 같은 질문을 해당 조문으로 연결한다.
- **대화가 남는다.** [LLM]은 로그인 후 채팅 기록·멀티턴 맥락·메시지별 근거를 영속화(SQLite)하고, 응답은 스트리밍(SSE)으로 받는다. 답변마다 👍/👎(+사유) 피드백을 남길 수 있다.
- **스스로 나아진다.** 거부·👎·인기 질문·콘텐츠 갭이 운영자 대시보드(`/admin`)와 검수 큐로 흘러, 무엇을 보강할지를 알려주는 자기개선 루프를 이룬다. (상세: [14-feedback-loop.md](14-feedback-loop.md))
...
- **관계가 한눈에.** [뇌]의 관계 그래프는 293 노드·357 연결을 4색(규정집 파랑·연구행정 가이드 초록·용어집 주황·사내 시스템 보라)으로 보여주며, 교차링크로 ERP 모듈이 허브가 된다. 둘러보기(/browse)에서는 4섹션·분류·검수상태 필터로 좁혀 찾는다.
- **눈이 편한 화면.** 웹앱은 라이트·다크·시스템 테마를 지원한다.

```mermaid
flowchart TD
    V["📁 단일 볼트 KEI-행정가이드/<br/>4개 섹션 · 293문서<br/>(Source of Truth · 마크다운)"]
    V --> Brain["🧠 [뇌] Next.js 14 + TDS<br/>둘러보기 + 관계 그래프<br/>(사람이 탐색)"]
    V --> Assist["🤖 [LLM] 멀티턴 RAG 채팅 + Ollama<br/>제N조 검색 + 근거주입(스트리밍)<br/>[규정명 제N조] 출처<br/>(행정 초보가 사용)"]
...
| G3 | 흩어진 HWP·가이드·용어·사내 시스템을 **조문 구조를 보존한 마크다운**(4섹션·293문서)으로 통합 | 변환·검수 완료 문서 수, 검수완료 비율(§5) |
...
| **거부율(모르면 모른다)** | 근거가 없는 질문에 "규정에서 확인되지 않습니다"로 답한 비율 | 의도적 답 없음/범위 밖 질문셋으로 측정 + 운영 중에는 대시보드가 거부 응답을 자동 감지 집계 | TODO |
| **검수완료 비율** | 전체 노트 중 `검수상태: 검수완료`인 비율(특히 `20_규정원문/`) | 프론트매터 `검수상태` 집계(Dataview/스크립트) | TODO |
| **사용 빈도** | 주간 질의 수, 활성 사용자 수, 질문 커버리지 | 운영자 대시보드(`/admin`, `GET /app/stats` 관리자 전용) 활동·인기질문·콘텐츠 갭 집계 | TODO |
...
- **모델·임베딩·벡터DB가 전부 사내에.** 격리 Ollama v0.31.1(LLM 서빙, OpenAI 호환 `127.0.0.1:11436/v1`, Qwen3.5-9B GGUF Q4_K_M)·`nlpai-lab/KURE-v1`(임베딩)·Chroma(벡터DB `kei_regs`)가 사내 GPU(Quadro RTX 6000 24GB×2) 위에서 돈다. 외부 API로 규정 텍스트를 보내지 않는다.
- **두 화면 모두 사내 전용.** [뇌] 둘러보기·관계 그래프, [LLM] 채팅 모두 Cloudflare Zero Trust(이메일 인증) 뒤에 둔다. 인터넷 공개 금지.
- **연결도 내부 경로.** 웹앱(`web/`)이 RAG API(`127.0.0.1:9000`)를 **같은 오리진 리버스 프록시**(`/api/rag/*`·`/api/app/*`)로 연결해, RAG API를 LAN에 노출하지 않는다. 상세는 [06-deployment.md](06-deployment.md).
...
flowchart LR
    subgraph net["사내망 · 인터넷 공개 금지"]
        ...
        ZT --> Web["🧠🤖 web/ Next.js+TDS<br/>둘러보기·관계 그래프·LLM 채팅"]
        Web --> RAG["RAG API (FastAPI · 127.0.0.1:9000)"]
        RAG --> EMB["임베딩 KURE-v1"]
        RAG --> DB["Chroma kei_regs"]
        RAG --> LLM["Ollama (OpenAI 호환)"]
