...
| **전입자** | 행정 경험은 있으나 KEI 규정 체계는 낯섦 | "**우리 기관에서는** 이걸 어느 규정이 정하나?" | [뇌] 관계 그래프로 규정·용어·ERP 관계 탐색, [LLM]으로 근거 확인 |
...
> **사내 규정·가이드·용어·ERP를 단일 마크다운 볼트(4개 섹션·271문서)로 모으고, 같은 볼트를 두 개의 화면 — 탐색용 그래프 [뇌]와 출처 달린 답변용 LLM [LLM] — 으로 제공하는 온프레미스 지식베이스 + 로컬 LLM.**

코퍼스는 네 개 섹션으로 이루어진다: **규정집**(`20_규정원문/`, 111문서) · **연구행정 가이드**(`10_업무가이드/`, 64문서) · **용어집**(`30_용어집/`, 84문서) · **ERP 시스템**(`40_시스템/`, 12문서) = 총 **271문서**. 섹션 간 교차링크(특히 ERP 모듈↔규정·용어)로 그래프 엣지를 만든다.
...
- **관계가 한눈에.** [뇌]의 관계 그래프는 271 노드·275 연결을 4색(규정집 파랑·연구행정 가이드 초록·용어집 주황·ERP 시스템 보라)으로 보여주며, 교차링크로 ERP 모듈이 허브가 된다. 둘러보기(/browse)에서는 4섹션·분류·검수상태 필터로 좁혀 찾는다.
- **눈이 편한 화면.** 웹앱은 라이트·다크·시스템 테마를 지원한다.

```mermaid
flowchart TD
    V["📁 단일 볼트 KEI-행정가이드/<br/>4개 섹션 · 271문서<br/>(Source of Truth · 마크다운)"]
...
| G3 | 흩어진 HWP·가이드·용어·ERP를 **조문 구조를 보존한 마크다운**(4섹션·271문서)으로 통합 | 변환·검수 완료 문서 수, 검수완료 비율(§5) |
...
- **모델·임베딩·벡터DB가 전부 사내에.** Ollama(LLM 서빙, OpenAI 호환 `127.0.0.1:11434/v1`, Qwen2.5-14B-Instruct Q4_K_M)·`nlpai-lab/KURE-v1`(임베딩)·Chroma(벡터DB `kei_regs`)가 사내 GPU(Quadro RTX 6000 24GB×2) 위에서 돈다. 외부 API로 규정 텍스트를 보내지 않는다. (vLLM은 대안 서빙 경로로만 둔다.)
