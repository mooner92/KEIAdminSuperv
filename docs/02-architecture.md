...
flowchart TD
    ...
    OLLAMA["격리 Ollama v0.31.1 (OpenAI 호환)<br/>Qwen3.5-9B Q4_K_M (unsloth)<br/>127.0.0.1:11436/v1 · PM2 kei-ollama-v031"]
...
> 변환·임베딩·검색·생성까지 실제 실행으로 검증되었습니다. 생성은 **격리 Ollama v0.31.1**(OpenAI 호환, `127.0.0.1:11436/v1`, PM2 `kei-ollama-v031`, ctx 8K, 모델 `Qwen3.5-9B Q4_K_M GGUF·unsloth`)로 구동되며 한국어 답변을 검증했습니다(vLLM 아님 — 이전 vLLM/Qwen2.5 계획을 대체). 리랭커(P1.4) 적용 후 평가 strict Hit@1 0.600→0.829, @5 1.000. 검색·근거주입·출처 표기·면책 보장도 검증되었습니다(상세: [04-pipeline.md](04-pipeline.md), [05-rag-design.md](05-rag-design.md), [12-품질강화.md](12-품질강화.md)).
...
sequenceDiagram
    ...
    participant L as 격리 Ollama v0.31.1 (Qwen3.5-9B)
...
>
> 생성은 **격리 Ollama v0.31.1**가 `Qwen3.5-9B Q4_K_M(GGUF, ~5.7GB, unsloth, apache-2.0)` 양자화 모델로 서빙합니다(단일 RTX 6000 24GB에 여유롭게 상주, GPU0가 비어 keep-alive 상주에 충분). ⚠ NVIDIA 드라이버 535라 v0.31.1은 CUDA 대신 Vulkan로 GPU를 사용합니다(550+ 업그레이드 시 자동 CUDA 전환·속도 개선). 임베딩(KURE-v1)은 1장으로 충분합니다(실측).
...
flowchart TB
    subgraph HOST["사내 서버 (Ubuntu) · 내부망"]
        subgraph GPU["GPU (Quadro RTX 6000 24GB×2)"]
            OLLAMA["격리 Ollama v0.31.1<br/>Qwen3.5-9B Q4_K_M<br/>:11436/v1 (keep_alive=-1)"]
            EMB["임베딩 KURE-v1"]
