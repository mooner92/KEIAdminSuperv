...
flowchart LR
    ...
    subgraph Asst["[LLM] Open WebUI + vLLM"]
        R["RAG API (가드레일/출처강제)"]
...
| **그래프뷰** (Graph view) | 노트를 점, 링크를 선으로 그린 시각화. 규정·가이드·용어가 어떻게 얽혀 있는지 한눈에 보여준다. | **주의:** [LLM]의 채팅은 이 그림을 읽지 않는다. 채팅은 텍스트+임베딩 검색으로 답한다. |
...
| **면책 문구** | RAG 답변 끝에 항상 붙이는 문장: **"최종 판단은 원문과 담당 부서 확인 바랍니다."** LLM의 답은 참고용이며 최종 근거는 원문임을 분명히 한다. | 가드레일 4번. → [05-rag-design.md](05-rag-design.md) |
...
> [!note] 같은 단어, 다른 자리
> "검색"이라는 말은 두 화면에서 다르게 쓰인다. [뇌] Quartz의 **전문검색**은 키워드 매칭이고, [LLM]의 검색은 **임베딩 기반 의미 검색**이다.
...
| 두 화면 표기 | **[뇌] Quartz** / **[LLM] Open WebUI+vLLM** | 항상 이 형태로 |
