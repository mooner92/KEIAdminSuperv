...
| **Ollama** | 본 프로젝트가 **실제로** LLM을 서빙하는 데 쓰는 추론 엔진. **OpenAI 호환 API**를 제공하므로 클라이언트가 OpenAI SDK 그대로 붙는다. 실측은 **격리 Ollama v0.31.1**(PM2 `kei-ollama-v031`, ctx 8K), 주소 `http://127.0.0.1:11436/v1`. | 모델은 일반 instruct(`Qwen3.5-9B` Q4_K_M, GGUF, unsloth). 코더/VL 모델 아님. ⚠ 공유 Ollama(`11434`, v0.24.0, 동료 운용)는 미사용. → [06-deployment.md](06-deployment.md) |
...
| **관계 그래프** (Graph view) | 노트를 점, 링크를 선으로 그린 시각화(`web/` `/graph`). 규정·가이드·용어·시스템이 어떻게 얽혀 있는지 한눈에 보여준다. | **주의:** [LLM]의 채팅은 이 그림을 읽지 않는다. 채팅은 텍스트+임베딩 검색으로 답한다. |
...
| **면책 문구** | RAG 답변 끝에 항상 붙이는 문장: **"최종 판단은 원문과 담당 부서 확인 바랍니다."** LLM의 답은 참고용이며 최종 근거는 원문임을 분명히 한다. LLM이 종종 누락하므로 `_ensure_disclaimer`로 100% 부착을 보장(스트리밍 포함). | 가드레일 4번. → [05-rag-design.md](05-rag-design.md), `../tools/rag_core.py` |
...
| 기본 LLM | `Qwen3.5-9B` (Q4_K_M, GGUF, unsloth) | 일반 instruct (코더/VL 아님). 격리 Ollama v0.31.1 구동 |
| LLM 서빙 엔진 | **격리 Ollama v0.31.1** (`127.0.0.1:11436/v1`) | 실측. vLLM은 대안 표기 |
