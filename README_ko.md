...
아래는 22개 화면 중 대표 4개입니다. **[→ 22개 화면 전체 사용법(GIF)](docs/73-화면-사용법.md)**

<img src="docs/img/gif/01-질문하기.gif" width="820" alt="질문하기 — 상황 칩을 고르거나 평소 말투로 질문을 입력해 근거가 달린 답변을 받는 흐름">

**질문하기** — 평소 말투로 물으면 사내 문서를 근거로 답하고, 답 끝에 <code>[규정명 제N조]</code> 출처가 붙습니다. 근거에 없으면 "규정에서 확인되지 않습니다"라고 답합니다.

<img src="docs/img/gif/03-문서-찾기.gif" width="820" alt="문서 찾기 — 검색어를 넣고 구분·분류·검수상태 필터로 좁히는 흐름">

**문서 찾기** — 구분·분류·검수상태 필터(패싯 카운트) + 제목·내용 전문검색. 행을 누르면 페이지 이동 없이 우측 드로어로 원문이 열립니다.

<img src="docs/img/gif/11-여비-계산기.gif" width="820" alt="여비 계산기 — 국내/국외·직급·출장 유형·일수를 골라 정액표를 확인하는 흐름">

**여비 계산기** — 조건을 고르면 **규정 별표의 정액을 원문 그대로** 보여 줍니다. 곱셈(정액×일수)만 하고, 실비 항목은 금액을 만들지 않고 <code>실비</code>로 표시합니다.

<img src="docs/img/gif/07-업무-한-장.gif" width="820" alt="업무 한 장 — 업무를 고르면 신청·결재·정산·보고 단계가 펼쳐지는 흐름">

**업무 한 장** — 출장·연차·법인카드 등 자주 하는 업무를 신청 → 결재 → 정산 → 보고까지 한 장으로. 단계마다 근거 조문·ERP 화면·필요 서식이 함께 붙습니다.

**그 밖의 화면** — 결재선 판정기(위임전결규정 별표 근거) · 업무 캘린더(이번 달 히어로 + 4×3 연간 그리드) · 기한 사전(기준일→마감일 계산, `.ics` 저장) · 서식 찾기(별지 PDF·HWP 다운로드) · 관계 그래프 · 개정 영향 분석 · 품질 게시판 · 실험실 · 관리자 대시보드. 조작 방법은 [docs/73](docs/73-화면-사용법.md)에서 화면별 GIF로 볼 수 있습니다.

> [!note]
> GIF에서 **규정 본문·금액·기한 수치·품질 통계·계정**이 읽히는 영역은 첫 프레임부터 흐리게 처리했습니다(공개 저장소이므로). 버튼·필터·탭 등 조작에 필요한 것은 모두 선명합니다.
> 정지 스크린샷(`docs/img/screen-*.png`)은 `web/shots-readme.mjs`가 실제 화면을 렌더해 만들며, **근거에 비공개 층(내부 가이드·ERP 문서)이 섞이면 실패**하도록 가드를 두고 있습니다.
...
| 73 | 화면 사용법 | 화면 22종 조작 흐름을 GIF로 (처음 쓰는 사람용) | [73-화면-사용법.md](docs/73-화면-사용법.md) |
