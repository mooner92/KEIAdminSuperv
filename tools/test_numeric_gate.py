...
def test_broken_exclusion_not_defeated_by_bracket_lines():
    """리뷰 C1: 표손상 블록 '본문'의 단독 '[별표 2]'/'[IMAGE]' 라인으로 제외가 풀리지 않는다."""
    ctx = (f"[상조회규약 별표1 {TABLE_BROKEN_MARK}]\n[별표 2]\n[IMAGE]\n사망 본인 : 3,000,000원"
           + SEP + "[복무규정 제16조]\n연 15일")
    n = numeric_guard_note("부모상 경조금?", "부모상 경조금은 300만 원입니다.", ctx)
    assert "3,000,000원" in n
...
# ── 적대 리뷰 재현 케이스(2차) ──────────────────────────────────────
def test_cheonman_baekman_money():
    """리뷰 major: '2천만원'·'5백만 원'·'1억 6천만원' 계열(계약 한도류 실코퍼스 표기)."""
    assert note("한도?", "한도는 20,000,000원입니다.", "[계약기준]\n계약 한도는 2천만원으로 한다.") == ""
    assert note("금액?", "계약금액이 500만 원 이상이면 심사 대상입니다.", "[계약사무처리규칙]\n계약금액이 5백만 원 이상인 경우") == ""
    assert note("한도?", "한도는 160,000,000원입니다.", "[지침]\n한도는 1억 6천만원으로 한다.") == ""
    n = note("한도?", "한도는 3천만원입니다.", "[계약기준]\n계약 한도는 2천만원으로 한다.")
    assert "30,000,000원" in n  # 조작 — 반드시 경고


def test_year_like_amounts():
    """리뷰 major: 무콤마 4자리 금액이 연도로 마스킹되던 결함 — 양방향."""
    assert note("수수료?", "수수료는 20000원입니다.", "[규칙]\n수수료는 20,000원으로 한다.") == ""
    n = note("수수료?", "수수료는 2000원입니다.", "[규칙]\n수수료 규정(금액 없음)")
    assert "2,000원" in n  # 조작 통과(FN) 금지
    assert note("연도?", "이 규정은 2024년에 개정되었습니다.", "[규칙]\n<개정 2024. 1. 1.> 조문") == ""  # 연도는 비검증


def test_dates_with_year_and_dots():
    """리뷰 major: 연도 포함 조작 날짜 차단 + 점표기 근거 날짜 인정."""
    n = note("마감?", "증빙 마감은 2026년 1월 18일입니다.", "[연말정산안내]\n마감 일정은 부서 공지 참조.")
    assert "1월 18일" in n  # 연도 붙여도 우회 불가
    assert note("마감?", "마감은 1월 18일까지입니다.", "[안내]\n제출기한: 2026. 1. 18.까지") == ""
    assert note("지급일?", "지급일은 12월 29일입니다.", "[안내]\n지급일: 12. 29.(월)") == ""


def test_unit_aware_matching():
    """리뷰 major: 근거의 '5일'이 답변의 '5%'를, '2년'이 '2개월'을 면제하지 않는다."""
    n = note("수수료율?", "수수료율은 5%입니다.", "[규칙]\n제출기한은 5일 이내로 한다.")
    assert "5%" in n
    n = note("임기?", "위원 임기는 2개월입니다.", "[규정]\n위원 임기는 2년으로 한다.")
    assert "2개월" in n


def test_calc_eq_split_and_operators():
    """리뷰 major: '=' 분할 파싱 — 결과 뒤 부연 숫자 무시·재언급 허용·잘못된 연산자 차단·나눗셈."""
    ctx = "[여비규정]\n숙박비 100,000원, 식비 25,000원, 일비 25,000원"
    q = "1박 2일 출장 다녀왔어요"
    assert note(q, "숙박비 100,000원 + 식비 25,000원 = 125,000원 (1박 2일 기준)입니다.", ctx) == ""
    assert note(q, "일비 25,000원 × 2일 = 50,000원입니다. 따라서 총 50,000원을 받습니다.", ctx) == ""
    n = note(q, "숙박비 100,000원 + 식비 25,000원 = 130,000원입니다.", ctx)
    assert "130,000원" in n  # 틀린 합계
    n = note(q, "식비 25,000원 + 2일 = 50,000원입니다.", ctx)
    assert "50,000원" in n  # 연산자-산술 불일치(25,000+2≠50,000)
    ctx2 = "[보수규정]\n월급여는 3,000,000원이며 일할계산한다(30일 기준)."
    assert note("일할계산?", "3,000,000원 ÷ 30일 = 100,000원입니다.", ctx2) == ""


def test_seq_column_not_harvested():
    """리뷰 major: 표의 번호·순번 열은 허용집합에 수확하지 않는다."""
    ctx = "[규정 별표]\n| 번호 | 항목 |\n| --- | --- |\n| 3 | 회의비 |\n| 7 | 국내여비 |"
    n = numeric_guard_note("보존기간?", "보존기간은 3년입니다.", ctx)
    assert "3년" in n


def test_range_restatement():
    """리뷰 minor: 근거 '3~5일'의 재서술('3일에서 5일')이 과차단되지 않는다."""
    assert note("휴가?", "청원휴가는 3일에서 5일입니다.", "[복무규정]\n청원휴가는 3~5일 부여한다.") == ""


def test_paren_annotation_prose():
    """리뷰 info: 산문 괄호 병기 '20(25)일'의 양쪽 값 인정."""
    assert note("연차?", "연차휴가는 20일(다태아 25일)입니다.", "[복무규정]\n출산휴가는 20(25)일 부여한다.") == ""
