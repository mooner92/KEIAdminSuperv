...
⚠ 색인제외 미적용(2026-07-25 실측 후 철회): 초기엔 파생 뷰라 RAG 색인에서 빼려 했으나,
  제외 시 실제 거부 회귀가 발생했다("임시계정 발급 정보" 질문이 v2 정상답변 → v3 거부).
  용어 노트는 **용어명이 제목**이라 검색 앵커 역할을 하고 있었다 — specs/02 §6 결론 참조.
  `색인제외` 메커니즘(02) 자체는 유지하되 이 노트들에는 쓰지 않는다.
