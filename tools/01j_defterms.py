...
# 별첨/별표/붙임 표제 — 이 지점부터는 인용된 외부 법령일 가능성이 커서 정의 스캔을 멈춘다.
_APPENDIX = re.compile(r'(?:^|\n)\s*(?:#{1,6}\s*)?[<\[(]?\s*(?:별\s*첨|별\s*표|별\s*지|붙\s*임)\s*\d*')
...
def extract(vault: str):
    ...
    for r in vp.iter_regulations(vault):
        for label, _title, body in r["articles"]:
            # ⛔ 별첨/별표/붙임 이후는 정의 스캔에서 뺀다 — 거기 실린 것은 대개 **다른 법령을
            #    인용한 조문**이라, 그 정의를 바로 앞 본문 조 번호에 귀속시키면 거짓 바인딩이 된다.
            #    실측(2026-08-06): '사적이해관계자'가 「퇴직자 활용에 관한 가이드라인」 제12조
            #    (수의계약 체결 제한)에 묶여, 정의형 라우팅이 엉뚱한 조문을 근거로 붙였고
            #    모델이 올바르게 거부했는데 평가는 '검색실패'로 기록했다(원인 7일간 은폐).
            body = _APPENDIX.split(body, 1)[0]
            found = []
