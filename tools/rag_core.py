...
# ⛔ **기각 — '리랭커 top-1 점수 임계값'을 제3의 부재 신호로 쓰는 안**(2026-08-19 실측).
#   08-18 결과지가 "코퍼스 밖 주제는 top-1 점수가 낮을 것"이라며 미측정으로 남긴 프로브를,
#   저장된 답변의 rerank 점수 4,105건(08-16~08-19 6회차)으로 GPU 없이 재측정했다.
#   ⓐ 분리력은 **실재한다**: 리랭커 top-1 중앙값이 거부형 정답 0.002 · 거부형 오답 0.008 vs
#      비거부형 정답 **0.585** — 두 자릿수 배율 차이다. 신호 자체는 있다.
#   ⓑ 그런데 **게이트로는 못 쓴다**: 임계값을 훑어도 최고 정밀도가 T=0.01에서 17.7%
#      (표적 재현 69.9% · 정상 답변 헛경고 **8.80%**). T=0.05면 재현 98.8%지만 헛경고 18.9%.
#      배포 중인 교집합 게이트(정밀도 48.0% · 헛경고 0.45%)보다 **20배 시끄럽다** — 정상 답변
#      5~11건에 1건꼴로 "근거 밖" 경고가 붙으면 경고 자체가 무의미해진다(알림 피로).
