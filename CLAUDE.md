...
- 여정신선도: `python tools/01k2_journey_freshness.py --vault KEI-행정가이드`  (업무 한 장 노드 근거 ↔ `article_status.json` 대조 → `journey_freshness.json`. 삭제/개정/미확인 분류·커버리지 동반 출력(별표는 감시 불가라 명시). 화면=플래그 `journey_freshness`, ⛔리포트만·수정은 사람. specs/13)
- 개정판별: `python tools/corpus_amend.py --file <변환본.md> [--doc <볼트문서.md>]`  (업로드가 **전문 개정본**인지 **개정안(신·구조문 대비표)**인지 결정적 판별 → 개정안이면 `corpus_replace.replace`가 **교체를 거부**한다. 실측 계기: 실제 개정 파일은 본문이 '생략/좌동'뿐이라 교체하면 651줄 규정이 51줄 요약본으로 덮이고 approval.json 335규칙이 사라진다. `--doc` 주면 바뀌는 문구가 볼트 **몇 번째 줄**인지까지 짚어준다. ⛔쓰기 경로 없음 — 회귀가 소스를 grep해 지킨다. 설계=specs/15)
- 개정반영: `python tools/corpus_amend_apply.py --vault <볼트> --doc <상대경로> --item <JSON>`  (대비표 한 줄을 볼트에 **전사**. 볼트에 쓰는 유일한 모듈 — 백업 우선·검수상태 미검수 복귀·개정일은 부칙 시행일. **신선도 재확인**: 줄 번호가 아니라 **내용 일치**가 조건이라 줄이 밀려도 틀린 줄을 안 덮는다(stale 거부·이미 반영은 already 멱등). 표·좌동·생략·모호·삭제는 관문에서 차단하고 **사유를 노출**한다. 실사용은 관리자 화면 `/admin` 코퍼스 탭(플래그 `corpus_amend`, ⛔일괄 반영 없음))
...
- 운영알림: `tools/alerts.py`(카탈로그 8종+유출 백스톱 `_scrub`) — 정본 `docs/66`. MaintNotice(🔔)+Slack `#horong`(Bot 토큰, ⚠맨 slack.com은 SNI 차단이라 `www.slack.com` 고정). 발생처 4곳+일일 다이제스트(`eval/eval_notice.py --digest`) 배선 완료, 회귀=`test_alerts.py`. 토큰=`ecosystem.local.js`(gitignore)
- 코드그래프: `scripts/graphify-refresh.sh [--label|--publish]` — graphify(전용 venv) 코드+문서 지식그래프, 정본 `docs/67`. ⛔유출 검사 내장·산출물 gitignore·라벨링은 사내 Ollama만. ⚠플래그는 등호형(`--backend=ollama`)만 파싱
- 실험실: `specs/09` — `/lab`(플래그 `lab_hub`·`lab_code_graph`, 기본 off) + server.js `/lab-assets/` 직서빙(게이트 뒤). 실험 등록은 4문항 게이트(졸업 기준 필수). 코드 그래프가 1호
- 출제필터: `eval/gen_filter.py`(3중 — 페르소나 1,176조합 프롬프트·결정적 결함사전 11종(한자혼입 포함)·LLM 검수자) + **일상어 패러프레이즈**(specs/11 — 골든 불변·질문만 눈 가리고 재작성, 게이트 4중: 결함·자족성·어휘겹침 0.55·뜻 보존 심판) — 2026-07-31 출제결함 7건 실측 후 개편, 회귀=`eval/test_gen_filter.py`(픽스처는 실측 발췌만)
