...
- LLM 서빙: 기존 vLLM(OpenAI 호환). 모델은 일반 instruct(Qwen2.5-14B-Instruct 등), 코더/VL 아님
  - GPU 메모리 주의: Qwen2.5-14B-Instruct fp16(약 28GB)은 RTX 6000 단일 24GB를 초과 → 2장 텐서병렬(`--tensor-parallel-size=2`) 또는 더 작은 instruct(7B/3B)·양자화 서빙이 필요. 임베딩(KURE-v1)은 1장으로 충분(실측).
- 비서 UI: Open WebUI(Docker). `tools/04_rag_api.py`를 OpenAI 호환 '모델'로 등록해 사용
- 그래프/문서 사이트: Next.js 14 + Toss Design System(`web/`, 정적 export → nginx). 정적 export(`next.config` `output:export`) → `out/` → nginx 127.0.0.1 → Cloudflare Zero Trust(서버 런타임 불필요, 기존 Quartz의 nginx/ZT 자리 대체). Pages Router·React 18 고정, `@toss/tds-mobile` v2.5.0. 컬러는 KEI 시맨틱 토큰(`web/styles/globals.css`). 디자인 규약은 `docs/design-system.md`. 실행: `cd web && VAULT_DIR=<볼트> npm run dev`(Node 22, http://127.0.0.1:3100)
